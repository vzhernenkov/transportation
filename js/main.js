...
function createTruck() {
  let name = document.querySelector("#truck__name").value,
    ...
    type = [...document.querySelectorAll(".form__radio")].filter(
      (el) => el.checked
    )[0].id;
...
async function edit() {
  ...
  let res = await editTruckInformation(
    editTruck,
    name,
    driver,
    number,
    location,
    type.getAttribute("id")
  );
  if (res.status == 200) {
    removeEditForm();
...
async function editTruckInformation(
  truck,
  newname,
  newdriver,
  newnumber,
...
async function deleteTruck(truck) {
  if (confirm("Are you sure, that you want to delete your truck?")) {
    let company = await getCurrentCompany().then((data) => data),
      ...
      truckName = currentTruckCard.querySelector(".truck__name").textContent;
    truckCardNumber =
      currentTruckCard.querySelector(".truck__number").textContent;
    company.trucks = company.trucks.filter(
      (el) => el.name !== truckName && el.truckNumber !== truckCardNumber
    );
...
async function editCompany(payload, key) {
...
async function validateRegistration(e) {
  ...
  let checkUser = await checkUserInUsers(user)
    .then((data) => data)
    .catch((err) => true);
  ...
  if (checkErrors(form) && !checkUser) {
    if (role == "carrier") {
      let carrierCompany;
      let checkCompany = await checkCarrierInCarriers(companyNumber).then(
        (data) => data
      );
      ...
      if (!checkCompany) {
        ...
        carrierCompany = await getCompanyByNumber(
          companyNumber,
          "carrier"
        ).then((data) => data);
      ...
      let shipperCompany;
      let checkCompany = await checkShipperInShippers(companyNumber).then(
        (data) => data
      );
      ...
      if (!checkCompany) {
        ...
        shipperCompany = await getCompanyByNumber(
          companyNumber,
          "shipper"
        ).then((data) => data);
